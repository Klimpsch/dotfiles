...
do
  ...
  --  Yanks (y) go to the system clipboard so you can paste elsewhere.
  --  Deletes are routed to the black-hole register by the mappings in
  --  Section 2, so `p` never pastes deleted text over your last yank.
  --  See `:help 'clipboard'`
...
do
  ...
  -- [[ Keep deletes out of the clipboard ]]
  -- Route delete/change/x to the black-hole register ("_) so they never
  -- populate the unnamed / system clipboard. Yank (y) still goes to the
  -- system clipboard via clipboard=unnamedplus, so `p` always pastes your
  -- last yank, never a delete.

  do
    local del_opts = { noremap = true, silent = true }
    for _, key in ipairs { 'd', 'D', 'x', 'X', 'c', 'C' } do
      vim.keymap.set({ 'n', 'v' }, key, '"_' .. key, del_opts)
    end
    -- The physical Delete key
    vim.keymap.set({ 'n', 'v' }, '<Del>', '"_x', del_opts)
    -- Visual-mode paste shouldn't clobber the clipboard with the replaced text
    vim.keymap.set('x', 'p', '"_dP', del_opts)
  end
...
do
  ...
  vim.api.nvim_create_autocmd('PackChanged', {
    callback = function(ev)
      ...
      -- markdown-preview.nvim needs its Node server built after install/update.
      -- Requires Node.js/npm on your PATH. Runs `yarn install` in the plugin's
      -- app/ folder via npx so you don't need yarn installed globally.
      if name == 'markdown-preview.nvim' then
        if vim.fn.executable 'npx' == 1 then
          run_build(name, { 'npx', '--yes', 'yarn', 'install' }, ev.data.path .. '/app')
        else
          vim.notify('markdown-preview.nvim: Node.js/npm not found on PATH; browser preview will not work.', vim.log.levels.WARN)
        end
        return
      end
    end,
...
do
  ...
  require('toggleterm').setup {
    ...
    size = 15, -- height in rows
  }
  -- also catch the <C-_> that some terminals send for Ctrl-/
  ...
  vim.keymap.set('n', '<leader>o', ':AerialToggle<CR>', { desc = 'Symbol [O]utline' })

  -- [[ Lualine statusline ]]
  -- NOTE: this replaces the mini.statusline set up in Section 4.
  vim.pack.add { gh 'nvim-lualine/lualine.nvim' }
  require('lualine').setup {
    options = {
      theme = 'nord', -- match your colorscheme
      icons_enabled = vim.g.have_nerd_font,
      section_separators = '', -- flat look; use '' for arrows if you prefer
  ...
  require('bufferline').setup {
    options = {
      ...
      offsets = { -- don't overlap the neo-tree sidebar
  ...
  vim.keymap.set('n', '<leader>bo', function()
    ...
    for _, b in ipairs(vim.api.nvim_list_bufs()) do
      if b ~= cur and vim.bo[b].buflisted then require('mini.bufremove').delete(b, false) end
    end
  end, { desc = 'Delete other buffers' })

-- Install nightfox via vim.pack
vim.pack.add({
...
require("nightfox").setup({
  options = {
    dim_inactive = true,   -- dim windows you're not in
    styles = {
      comments = "italic",
      keywords = "NONE",   -- less emphasis = calmer
    },
  },
})

-- Pick a muted variant:
vim.cmd.colorscheme("nordfox")   -- or terafox / carbonfox / duskfox
vim.pack.add({
  { src = "https://github.com/echasnovski/mini.indentscope", version = "stable" },
})


-- Install zen-mode and twilight via vim.pack
vim.pack.add({
...
vim.keymap.set("n", "<leader>z", function()
  vim.ui.select(
    { "Zen Mode", "Twilight", "Both" },
    { prompt = "Focus mode:" },
    function(choice)
      if choice == "Zen Mode" then
        vim.cmd("ZenMode")
...
end, { desc = "Focus mode menu" })
  -- [[ MARKDOWN: in-buffer rendering ]]
  -- Renders markdown directly in the buffer as you edit (headings, code blocks,
  -- lists, tables, etc.) with no browser and no build step. It relies on the
  -- `markdown` and `markdown_inline` treesitter parsers already installed in
  -- Section 9, so it works out of the box.
  vim.pack.add { gh 'MeanderingProgrammer/render-markdown.nvim' }
  require('render-markdown').setup {}
  -- Toggle the rendering on/off for the current buffer.
  vim.keymap.set('n', '<leader>mp', '<cmd>RenderMarkdown toggle<CR>', { desc = '[M]arkdown [P]review (in-buffer) toggle' })

  -- [[ MARKDOWN: browser preview ]]
  -- Opens a live-synced rendered preview in your default web browser.
  -- Requires Node.js/npm on your PATH; the Node server is built by the
  -- PackChanged autocmd in Section 3 after install/update.
  vim.pack.add { gh 'iamcco/markdown-preview.nvim' }
  vim.g.mkdp_filetypes = { 'markdown' }
  -- vim.g.mkdp_auto_start = 0   -- set to 1 to auto-open the browser on entering a markdown file
  -- vim.g.mkdp_auto_close = 1   -- close the browser tab when leaving the markdown buffer
  vim.keymap.set('n', '<leader>mb', '<cmd>MarkdownPreviewToggle<CR>', { desc = '[M]arkdown [B]rowser preview toggle' })
